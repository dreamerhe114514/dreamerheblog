...
categories:
  - 魔改
  - 美化
cover: /cover/mogaidiary .png
abbrlink: 57581
date: 2024-01-17 15:59:02
---
## 前言
这里汇聚了**所有**小何能找到的，并且用上的魔改教程!!!
## 语雀链接卡片
此教程来自于 
{% link https://meuicat.com,MeuiCat,https://meuicat.com/media/favicon.ico, 有肉有猫有生活 %}
### 创建数据
在butterfly主题文件夹的/scripts/tag/下创建link.js文件，并复制以下代码
``` js
/*
 * link
...
### 创建样式文件
在butterfly主题文件夹的/source/css/_tags/下创建link.styl
``` styl
.link_card
  display: flex
  margin: 10px 0
...
此教程来自于 
{% link https://meuicat.com,MeuiCat,https://meuicat.com/media/favicon.ico, 有肉有猫有生活 %}
{% link https://akilar.top/,Akilar,https://npm.elemecdn.com/akilar-friends@latest/avatar/akilar.top.jpg, 欢迎光临糖果屋 %}
但是此次魔改小何很懵，所以找了朋友王九弦，感谢九弦的大力支持!
{% link https://blog.sinzmise.top/,九弦之屋,https://cdn.sinzmise.eu.org/images/avatar.png, 随着风和光来的旅行者，欢迎来到九弦之屋 %}
### 创建数据

将[blogRoot]/themes/butterfly/layout/includes/loading/index.pug的内容替换成以下内容

``` jade
if theme.preloader.enable
  case theme.preloader.load_style    
    when 'fullpage-loading'
      include ./load_style/fullpage-loading.pug
    when 'icat'
      include ./load_style/icat.pug
  if theme.preloader.pace
    include ./pace.pug
```
在 [blogRoot]/themes/butterfly/source/css/新建个名字叫_load_style的文件夹

将 [blogRoot]/themes/butterfly/source/css/_layout/loading.styl 复制到上面新建的_load_style文件夹

然后将原先文件夹里面的loading.styl修改如下：

```stylus
if hexo-config('preloader.enable')
  if hexo-config('preloader.load_style') == 'fullpage-loading'
    @import './_load_style/fullpage-loading'
  else if hexo-config('preloader.load_style') == 'icat'
    @import './_load_style/icat'
```

在上面新建的_load_style文件夹里面新建个名字叫`icat.styl`的文件，内容如下：
...
在[blogRoot]/themes/butterfly/layout/includes/loading/新建个名字叫`load_style`的文件夹，把`fullpage-loading.pug`复制到新建的文件夹里面去

再在上个步骤新建的文件夹里面新建一个叫`icat.pug`的文件，内容如下：
...
编辑主题目录下的_config.yml（或者博客目录下的_config.butterfly.yml），找到preloader这一行，将preloader里面的整个内容替换如下：
...
preloader:
  enable: true
  # source
  # 例：fullpage-loading/icat
  load_style : icat
  # pace theme (see https://codebyzach.github.io/pace/)
  pace: true
  pace_css_url:
  logo: /img/dreamerheload.png #修改成你要的图片
  logo_dark: /img/dreamerheloaddark.png #修改成你要的图片
``````

## HEXO跳转第三方提示
这篇教程来自于{% link https://www.yichkhun.com,苏易困,https://cravatar.cn/avatar/32cb16db0a9e9d76daa10bea336def62?d=identicon,HEXO跳转第三方 %}

### 新建中转界面
我们新建一个中转页面，照这个大佬的说法，也就是redirect
``` npm
hexo new page redirect
```
然后我们将目录下的index.md文件删除，新建index.html
复制以下代码
``` html
<!DOCTYPE html>
<html lang="en">
...
<div class="box">
    <div class="tip-box">
        <div class="content">
            <div class="content">
                <div class="title">
				    即将离开 <a href="https://www.hexo.dreamerhe.qianxun7747.eu.org" target="_blank" id="site-name">Dreamerhe</a>，前往第三方网站，请注意账号财产安全
                    即将离开 <a href="https://www.hexo.dreamerhe.online" target="_blank" id="site-name">Dreamerhe</a>，前往第三方网站，请注意账号财产安全
                </div>
                <div id="link"></div>
...
```
再进入根目录的_config.yml配置文件中，搜索**skip_render**
``` yml
skip_render:
+  - "redirect/**"
```
## 替换超链接
再**theme/butterfly/layout/post.pug**页面添加如下内容（域名自行修改）
``` pug
    include includes/header/post-info.pug
   -
     const content = page.content;
     const urlRegExp = /(https?:\/\/(?!www\.)[^/\s]*\.\w+(?:\.\w+)*|https?:\/\/www\.(?!hexo.dreamerhe.qianxun7747\.eu.org)[^/\s]*\.\w+(?:\.\w+)*)/gi;
     page.content = content.replace(urlRegExp, (url) => {
         ...
         if (domainMatch !== null && domainMatch[1].indexOf('hexo.dreamerhe.eu.org') === -1) {
             return `http://www.hexo.dreamerhe.qianxun7747.eu.org/redirect/?target=${url}`;
         }
         return url;
     });
   -
